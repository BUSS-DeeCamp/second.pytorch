...
class Second3DDector(object):
    ...
    @staticmethod
    def load_pc_from_kitti_file(pc_f):
        logging.info('loading pc from: {}'.format(pc_f))
    ...
    def predict_on_nuscenes_local_file(self, v_p):
        tic = time.time()
        points = self.load_pc_from_nuscenes_file(v_p)[:, :4]
    ...
    def predict_on_kitti_local_file(self, v_p):
        tic = time.time()
        points = self.load_pc_from_kitti_file(v_p)[:, :4]
        print('points shape: ', points.shape)
        example = self.load_an_in_example_from_points(points)
        pred = self.net(example)[0]
        box3d = pred['box3d_lidar'].detach().cpu().numpy()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        logging.error('send a lidar file pls.')
    else:
        # config_p = 'second/configs/nuscenes/all_pp.nu.config'
        config_p = './second/configs/car.lite.config'
